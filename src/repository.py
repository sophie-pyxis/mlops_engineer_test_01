...
class PassengerRepository:
    ...
    def save(self, passenger_id: str, features: list, probability: float) -> None:
        """Persiste a predição gerada pelo modelo.
        Features e probabilidade são convertidas para string pois o DynamoDB
        não aceita float nativo do Python — apenas Decimal ou string."""
        logger.info(f"Persistindo passageiro {passenger_id} no DynamoDB.")
        self.table.put_item(Item={
            'id': passenger_id,
            'caracteristicas': [str(f) for f in features],
